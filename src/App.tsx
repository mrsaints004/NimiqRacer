...
// Three.js + the game scene is the bulk of the bundle — load it only once the player actually
// starts playing, instead of blocking the initial page paint (username/car-select screens) on it.
...
// Format Luna as NIM for display (100000 Luna = 1 NIM)
...
const CAR_OPTIONS: CarOption[] = [
  ...
  // Premium cars (locked until purchased)
...
function getUnlockedCars(deviceId: string | null): string[] {
  if (!deviceId) return [];
  try {
    const raw = localStorage.getItem(`miniracer_unlocked_${deviceId}`);
...
function saveUnlockedCar(deviceId: string, hex: string) {
  const unlocked = getUnlockedCars(deviceId);
  if (!unlocked.includes(hex)) {
    unlocked.push(hex);
    localStorage.setItem(`miniracer_unlocked_${deviceId}`, JSON.stringify(unlocked));
  }
}
...
function GameWrapper() {
  ...
  useEffect(() => {
    setUnlockedCars(getUnlockedCars(deviceId));
  }, [deviceId]);

  const isCarAvailable = useCallback(
    (car: CarOption) => !car.premium || unlockedCars.includes(car.hex),
    [unlockedCars]
  );
  ...
  const handleCarClick = useCallback(
    async (index: number) => {
      ...
      // Free car or already unlocked — just select it
      if (isCarAvailable(car)) {
        setSelectedCar(index);
        return;
      }

      // Premium car — trigger purchase
      ...
      if (result.success && deviceId) {
        saveUnlockedCar(deviceId, car.hex);
        setUnlockedCars(getUnlockedCars(deviceId));
        setSelectedCar(index);
      } else {
      ...
      >
        ...
        {!showLeaderboard && step === "car_select" && (
          ...
          >
            ...
            <button
              onClick={() => {
                if (isCarAvailable(CAR_OPTIONS[selectedCar])) {
                  setStep("playing");
                }
              }}
